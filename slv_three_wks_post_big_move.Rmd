...
A daily price change was significant if SLV's closing price moved up or down at least 3% from the previous close.
...
silver_tbl %>% filter(!is.na(big_move)) %>% count(big_move) %>% mutate(Percentage = round(100 * prop.table(n), 1)) %>% kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
Below is a plot of the simple return over 15 trading days (typically three weeks, ignoring holidays) following a daily price change +/- 3%, as well as the simple return following a small daily price change. The boxplot overlay shows the 25th, 50th and 75th percentiles for returns observed. (Obviously, the returns for the most recent observations are as yet unknown because 15 days haven't elapsed.)
